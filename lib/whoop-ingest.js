...
// ── Local-time helpers ───────────────────────────────────────────────────────
// Shift a UTC instant by a "+HH:MM"/"-HH:MM" offset so the resulting Date's
// UTC fields read as the local wall clock.
function shiftToLocal(iso, offset) {
  if (!iso || !offset) return null
  const t = new Date(iso).getTime()
  if (!Number.isFinite(t)) return null
  const sign = offset[0] === '-' ? -1 : 1
  const [oh, om] = offset.slice(1).split(':').map(Number)
  return new Date(t + sign * (oh * 60 + om) * 60000)
}
...
export async function mergeAndUpsert(rows) {
  ...
  const { data: existingRows, error } = await supabaseServer
    .from('whoop_data').select('*').in('date', dates)
  ...
  const { error: upErr } = await supabaseServer
    .from('whoop_data').upsert(merged, { onConflict: 'date' })
  if (upErr) throw upErr
  return { upserted: merged.length, dates }
}

// ── Orchestration ────────────────────────────────────────────────────────────
async function ingestRecords(recoveries, sleeps, cycles) {
  const { rows, skippedNoSleep, skippedNap, skippedNoDate } =
    buildRows(recoveries, indexById(sleeps), indexById(cycles))
  const { upserted, dates } = await mergeAndUpsert(rows)
...
export async function ingestFromWhoop(token, { maxPages = 200 } = {}) {
  ...
  return ingestRecords(recoveries, sleeps, cycles)
}

// Windowed re-ingest for webhooks — a bounded recent slice, same join+merge.
export async function ingestWindow(token, days = 7) {
  const end   = new Date()
