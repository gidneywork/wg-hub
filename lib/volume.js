...
// Returns average weekly tonnage per muscle over the `weeksBack` ISO weeks prior to `weekStartDate`.
// A week with no lifts counts as zero (rest weeks lower the baseline — correct behaviour).
// If fewer than `weeksBack` prior weeks of data exist, averages across all of logs history,
// still dividing by `weeksBack` (conservative — damps toward zero for new users).
export function computeMuscleBaseline(logs, weekStartDate, weeksBack = 4) {
  const muscleGroups = getMuscleGroups().filter(m => m !== 'cardio')
  const totals = Object.fromEntries(muscleGroups.map(m => [m, 0]))
  ...
  for (let i = 1; i <= weeksBack; i++) {
    ...
    const priorWeekStart = `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}-${String(d.getDate()).padStart(2, '0')}`
    const { byMuscle } = computeWeeklyMuscleVolume(logs, priorWeekStart)
    for (const m of muscleGroups) {
      totals[m] += byMuscle[m]?.tonnage ?? 0
    }
  }

  return Object.fromEntries(muscleGroups.map(m => [m, totals[m] / weeksBack]))
}

// Returns 'heavy' when this week's tonnage exceeds both an absolute floor and 1.5× the baseline.
// baselineTonnage > 0 guard prevents flagging a muscle with no training history.
export function classifyMuscleLoad(thisWeekTonnage, baselineTonnage) {
  if (thisWeekTonnage > 2000 && baselineTonnage > 0 && thisWeekTonnage > 1.5 * baselineTonnage) return 'heavy'
  return 'normal'
}
