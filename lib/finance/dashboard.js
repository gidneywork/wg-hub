...
import { billMonthlyEquivalent, BILL_TYPE_LABELS } from './bills'
 ...
 */
export function computeNetWorth(accounts, allTransactions, statementDocs, properties = [], debtRows = []) {
  let assets = 0, debt = 0
  ...
  for (const prop of properties) {
    assets += Number(prop.value_pence ?? 0)
  }

  // account_id set = linked revolving/overdraft; liability is the negative account balance above.
  // account_id null = manual/loan/mortgage; not in any account — must subtract explicitly.
  for (const row of debtRows) {
    if (row.account_id != null) continue
    debt += Number(row.balance_pence ?? 0) + Number(row.arrears_pence ?? 0)
  }
...
/**
 * Groups active bills by type and returns monthly totals, sorted descending.
 * @param {Array} bills  finance_bills rows (may include inactive)
 * @returns {Array<{ type: string, label: string, monthly: number }>}
 */
export function computeMonthlyBillsByType(bills) {
  const byType = {}
  for (const b of bills) {
    if (!b.is_active) continue
    const key = b.bill_type || 'other'
    byType[key] = (byType[key] ?? 0) + billMonthlyEquivalent(b)
  }
  return Object.entries(byType)
    .filter(([, v]) => v > 0)
    .sort(([, a], [, b]) => b - a)
    .map(([type, monthly]) => ({ type, label: BILL_TYPE_LABELS[type] ?? type, monthly }))
}

/**
 * Sums monthly debt commitments across all debt rows.
 * Revolving debts use min_payment_pence; loans/mortgages use monthly_payment_pence.
 * Context-only — does not imply these overlap with or duplicate bills.
 * @param {Array} debtRows  finance_debts rows
 * @returns {number}  Total monthly commitment in pence
 */
export function computeMonthlyDebtCommitments(debtRows) {
  return debtRows.reduce((sum, row) => {
    const debtType = row.debt_type ?? 'revolving'
    if (debtType === 'revolving') return sum + (Number(row.min_payment_pence) || 0)
    return sum + (Number(row.monthly_payment_pence) || 0)
  }, 0)
}
