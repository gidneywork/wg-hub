...
/**
 * Converts an annual rate in basis points to a monthly multiplier.
 * Returns null for flat-rate debts — callers must branch (flat interest is
 * pre-computed at origination, not applied month-by-month on a reducing balance).
 * @param {number|null} bps  Annual rate in basis points
 * @param {'apr'|'ear'|'flat'} [rateType='apr']
 * @returns {number|null}
 */
export function monthlyRateFromBps(bps, rateType = 'apr') {
  const annual = (bps ?? 0) / 10000
  if (rateType === 'ear')  return Math.pow(1 + annual, 1 / 12) - 1
  if (rateType === 'flat') return null
  return annual / 12  // apr (default) — byte-identical to existing callers
}
    ...
    if (linkedRow) {
      ...
      if (debtType === 'revolving') {
        ...
        result.push({
          ...
          rateType:        linkedRow.rate_type ?? 'apr',
...
export function simulateStrategy(debts, monthlyBudgetPence, strategy, maxMonths = 600) {
  ...
  while (month < maxMonths) {
    ...
    // Accrue interest — flat-rate debts accrue 0 (interest is pre-fixed, not reducing-balance)
    for (const d of active) {
      const r              = monthlyRateFromBps(d.aprBps, d.rateType)
      const interest       = r != null ? Math.round(d.balanceRemaining * r) : 0
 ...
 */
export function computeAmortisationPayment(principalPence, aprBps, months, rateType = 'apr') {
  if (months <= 0) return 0
  if (rateType === 'flat') {
    const totalInterest = Math.round(principalPence * (aprBps ?? 0) / 10000 * months / 12)
    return Math.round((principalPence + totalInterest) / months)
  }
  const r = monthlyRateFromBps(aprBps, rateType)
...
export function projectAmortisation(
  ...
  // Flat-rate preamble: total interest is fixed at origination; override payment;
  // disable fixed/revert (flat loans have no SVR step-up).
  let flatMonthlyInterest = null
  if (rateType === 'flat') {
    fixedPeriodMonths = null
    revertAprBps      = null
    const flatTotalInterest = Math.round(principalPence * currentAprBps / 10000 * termMonths / 12)
    payment             = computeAmortisationPayment(principalPence, aprBps, termMonths, 'flat')
    flatMonthlyInterest = Math.round(flatTotalInterest / termMonths)
  }

  // Feasibility guard: if first-month interest ≥ payment, balance can only grow.
  const firstInterest = flatMonthlyInterest !== null
    ? flatMonthlyInterest
    : Math.round(balance * (monthlyRateFromBps(currentAprBps, rateType) ?? 0))
  ...
  while (balance > 0 && month < maxMonths) {
    ...
    if (
      ...
      payment = computeAmortisationPayment(balance, revertAprBps, remainingTerm, rateType)
      recalculatedPayment = payment
      revertMonth = month
    }

    let interest
    if (flatMonthlyInterest !== null) {
      interest = flatMonthlyInterest
    } else {
      const r = monthlyRateFromBps(currentAprBps, rateType) ?? 0
...
/** Formats bps as a human-readable rate string with optional type suffix. 2290 → "22.9%" */
export function formatApr(bps, rateType = 'apr') {
  if (bps == null) return '—'
  const pct = bps / 100
  if (rateType === 'ear')  return `${pct}% EAR`
  if (rateType === 'flat') return `${pct}% flat`
  return `${pct}%`
}

/**
 * Builds chart data for the avalanche vs snowball comparison.
 * Sums balances[] per schedule entry — schedule has no totalRemaining field.
 * Pads the shorter strategy with 0 once it clears.
 */
export function buildComparisonChartData(avalancheResult, snowballResult, startTotalPence) {
  if (!avalancheResult.feasible || !snowballResult.feasible) return []
  const maxMonths = Math.max(avalancheResult.months ?? 0, snowballResult.months ?? 0)
  if (maxMonths === 0) return [{ month: 0, avalanche: 0, snowball: 0 }]

  const data = [{ month: 0, avalanche: startTotalPence, snowball: startTotalPence }]

  for (let m = 1; m <= maxMonths; m++) {
    const aSched = avalancheResult.schedule[m - 1]
    const sSched = snowballResult.schedule[m - 1]
    const aTotal = aSched ? aSched.balances.reduce((s, b) => s + b.balanceRemaining, 0) : 0
    const sTotal = sSched ? sSched.balances.reduce((s, b) => s + b.balanceRemaining, 0) : 0
    data.push({ month: m, avalanche: aTotal, snowball: sTotal })
  }
