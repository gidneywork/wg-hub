...
/**
 * Validates a custom category add form. Returns error string or null if valid.
 * @param {{ name: string }} form
 * @param {Array<{ name: string }>} existingCategories  All categories (seeded + custom).
 * @returns {string|null}
 */
export function validateCustomCategoryForm(form, existingCategories) {
  const trimmed = form.name?.trim() ?? ''
  if (!trimmed) return 'Name is required.'
  if (trimmed.length < 2) return 'Name must be at least 2 characters.'
  if (trimmed.length > 60) return 'Name must be 60 characters or fewer.'
  const lower = trimmed.toLowerCase()
  if (existingCategories.some(c => c.name.toLowerCase() === lower)) {
